...
class NewsAdmin(admin.ModelAdmin):
    form = NewsAdminForm
    list_display = ('title', 'pretty_description', 'publish_date', 'is_archived', 'action_buttons')
    search_fields = ('title', 'description')
    actions = ['archive_selected']
    change_list_template = 'admin/news/news_changelist.html'

    def pretty_description(self, obj):
        # Remove HTML tags for tooltip, but keep them for display
        plain_text = re.sub('<[^<]+?>', '', obj.description)
        truncated_html = Truncator(obj.description).chars(120, html=True, truncate=' ...')
        return format_html(
            '<div style="max-width:350px; white-space:pre-line;" title="{}">{}</div>',
            plain_text.replace('"', '&quot;'),
            mark_safe(truncated_html)
        )
    pretty_description.short_description = 'Description'
